"""ML Inference using TEST.py ensemble predictions."""
import os
import sys
import torch
...
def load_model_and_modify(model_name: str, num_classes: int, device: torch.device):
    """Load and modify model architecture to match training."""
    if model_name == "densenet121":
...
def visualize_gradcam(original_img, cam, title="Grad-CAM"):
    """Visualize Grad-CAM on original image."""
...
def predict_ensemble(image_bytes: bytes, include_gradcam: bool = False) -> Dict:
    """
    Predict using ensemble of all available models with TEST.py logic.
    Returns prediction with confidence scores and optional Grad-CAM visualization.
    """
    ...
    # Majority voting as secondary ensemble method
    ...
    # Add Grad-CAM visualization if requested
